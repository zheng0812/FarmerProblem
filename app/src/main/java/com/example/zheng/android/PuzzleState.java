...
//        return tilesOutOfPlace(this, goal);
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        int count = 0;
        PuzzleState curren= (PuzzleState) current;
        PuzzleState go= (PuzzleState) goal;
        if(!curren.getLocation(1).equals(go.getLocation(1))){
            count++;
        }
        else if(!curren.getLocation(2).equals(go.getLocation(2))){
            count++;
        }
        else if(!curren.getLocation(3).equals(go.getLocation(3))){
            count++;
        }
        else if(!curren.getLocation(4).equals(go.getLocation(4))){
            count++;
        }
        else if(!curren.getLocation(5).equals(go.getLocation(5))){
            count++;
        }
        else if(!curren.getLocation(6).equals(go.getLocation(6))){
            count++;
        }
        else if(!curren.getLocation(7).equals(go.getLocation(7))){
            count++;
        }
        else if(!curren.getLocation(8).equals(go.getLocation(8))){
            count++;
        }
        else if(!curren.getLocation(9).equals(go.getLocation(9))){
            count++;
        }
        /* compute the count */
        return count;
    ...
    public static int sumManhattan(State current, State goal) {
        ...
        /* compute the sum */
        PuzzleState curren= (PuzzleState) current;
        PuzzleState finalState = (PuzzleState) goal;
         

        for (int row = 0; row < curren.tiles.length; row++) {
            for (int col = 0; col < curren.tiles[row].length; col++) {
                int value = curren.tiles[row][col];
                int temp = finalState.tiles[row][col];
                if(value !=0 && value != temp){
                    sum += Math.abs(row- finalState.getLocation(value).getRow()) + Math.abs(col - finalState.getLocation(value).getColumn());
                }
        }
    }
        return sum;
